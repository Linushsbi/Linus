...
@PageTitle("Mitglieder Verwalten")
@Route(value = "mitglieder-verwalten")
@RolesAllowed({ "ADMINISTRATOR" , "GESCHAEFTSFUEHRER" , "MITARBEITER" })
public class MitgliederVerwalten extends VerticalLayout {
    private final UserService userService;
    private final Grid<User> grid;
    private final AuthenticatedUser authenticatedUser;

    @Autowired
    public MitgliederVerwalten(UserService userService, AuthenticatedUser authenticatedUser) {
        this.userService = userService;
        this.authenticatedUser = authenticatedUser;
        this.grid = new Grid<>(User.class);

        // Create a title for the view
        H1 title = new H1("Mitglieder Verwalten");
        ...
        List<User> mitarbeiterList = userService.findAllMitglieder();
    ...
    private void navigateToDashboard() {
        ...
        if (currentUser.isPresent()) {
            ...
            } else if ("mitarbeiter" .equals(role)) {
                UI.getCurrent().navigate("mitarbeiter-dashboard");
            }
        }
    }
    ...
    private void openDeleteConfirmationDialog(User user) {
        ...
        dialogLayout.add("Möchten Sie dieses Mitglied wirklich löschen?");
        Button confirmButton = new Button("Löschen" , event -> {
            userService.deleteUserById(user.getId());
            grid.setItems(userService.findAllMitglieder());
            Notification.show("Mitglied gelöscht" , 3000, Notification.Position.MIDDLE);
            dialog.close();
        });
